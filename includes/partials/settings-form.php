...
?>
		...
		<td>
			...
			<?php
			foreach ( array( 5, 10, 20, 50, 100, 500 ) as $wpff_sp_batch_size ) {
				printf(
					'<option value="%s"%s>%s</option>',
		...
		<td>
			...
			<?php
			foreach ( array( 1, 2, 3, 5, 10 ) as $wpff_sp_delay_between_urls ) {
				printf(
					'<option value="%s"%s>%s</option>',
