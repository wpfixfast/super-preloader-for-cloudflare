...
class WPFF_SP_Preloader {
	...
	public static function run( $is_continuation = false ) {
		...
		if ( $cursor['index'] >= count( $queue ) ) {
			...
			$duration_string  = null !== $duration_seconds ? self::format_duration( $duration_seconds ) : __( 'Unknown', 'super-preloader-for-cloudflare' );
			...
			if ( $full_proxy_pass ) {
				...
				$edges       = isset( $stats['_edges'] ) ? $stats['_edges'] : array();

				update_option(
					'wpff_sp_preload_stats',
					array(
				...
				$previous = get_option( 'wpff_sp_preload_stats', array() );
			 ...
			 */
			do_action( 'wpff_sp_preloader_completed', get_option( 'wpff_sp_preload_stats', array() ) );
			...
			set_transient( 'wpff_sp_preload_cursor', $cursor, 24 * HOUR_IN_SECONDS );
			wp_schedule_single_event( time() + 2, 'wpff_sp_run_preloader', array( true ) );
